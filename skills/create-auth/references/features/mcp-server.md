...
1. Read `Authorization: Bearer <token>` header (or `Authorization: DPoP <token>` if you implement the optional sender-constraining under **Best Practices**). If missing or malformed → `401` with `WWW-Authenticate` (see below). No fallback to query string or cookie for MCP endpoints.
...
- MUST validate the body is JSON and contains `client_id`, `client_name`, `redirect_uris`. Everything else (`logo_uri`, `client_uri`, `grant_types`, `token_endpoint_auth_method`) is optional and gets the same validation you apply on DCR — scheme rules, no fragments, `application_type` sanity — with one deliberate difference: an **omitted** `token_endpoint_auth_method` in a metadata document means `none`, and any `client_secret_*` value is rejected (the CIMD draft forbids shared-secret methods — a document is public, so there is nobody to hand a secret to). The RFC 7591 `client_secret_basic` default below applies to DCR only. A document naming `private_key_jwt` is rejected as unsupported by this spec, never silently downgraded to `none`.
...
- **Validation rules:**
  ...
  - **Confidentiality comes from exactly one stored field:** a client is confidential iff `tokenEndpointAuthMethod !== "none"`. Derive every downstream decision from that single read — whether registration issues a `client_secret`, whether `/oauth/token` demands one. Never keep a parallel `public` boolean, and never infer it from `clientSecretHash IS NULL`. Two fields encoding one fact drift, and the permissive reading wins: a client registered `client_secret_basic` whose secret row is missing silently degrades into a public client that skips authentication entirely. Application type and authentication method are orthogonal axes — a `native` client may register `client_secret_basic`, a `web` client may register `none`; never let one field imply the other. When `token_endpoint_auth_method` is `none` (the common case for desktop MCP clients), issue no `client_secret`; PKCE alone authenticates. When it is **omitted**, the RFC 7591 §2 default is `client_secret_basic` — register a confidential client and issue a secret. Do not downgrade an omitted method to `none` "because the registration was unauthenticated": that turns every anonymous registration into a public client, and a client that then sends the secret it never got is silently skipping authentication rather than failing (better-auth 1.7 fixed exactly this default). Expect the correct default to surface in clients that never read `token_endpoint_auth_method` back from the registration response and then authenticate through the body — they now get `invalid_client`, which is the RFC-correct answer, not a bug to paper over. Reject any value outside `none` and `client_secret_basic` — the two this spec advertises in `token_endpoint_auth_methods_supported`; add `client_secret_post` only if you also advertise it and enforce it as its own channel below.
...
- **Validation order — short-circuit on first failure, and do NOT redirect on errors that occur before `redirect_uri` is validated** (otherwise an attacker can use you as an open redirect):
  ...
  3. From here on, errors redirect to the now-verified `redirect_uri` with `error=` (and `state`, and `iss`): a *missing* `response_type` is `invalid_request` and an unsupported one (`token`, `id_token`, …) is `unsupported_response_type` — both *on the redirect*, not a bare 400, because the client is known and its callback is verified (RFC 6749 §4.1.2.1). `code_challenge_method` MUST be `S256` (`invalid_request` otherwise), and `code_challenge` MUST be present.
  ...
  6. Reject the OIDC `request` and `request_uri` parameters explicitly — `error=request_not_supported` / `request_uri_not_supported` (OIDC Core §3.1.2.6) — rather than ignoring them. A client that sends a request object believes the parameters inside it are the signed, authoritative ones; an AS that silently reads the plain query parameters instead is honouring a request the client never made. If you also serve `/.well-known/openid-configuration`, advertise `request_parameter_supported: false` and `request_uri_parameter_supported: false` — the OIDC Discovery default for the latter is `true`.
...
- **The consent screen's own approval is an HTML form navigation, and its URL carries `code_challenge`/`state`.** Those two facts fight each other: the query argues for `Referrer-Policy: no-referrer` on this page, and `no-referrer` makes the browser send `Origin: null` on the form POST the approve button navigates with (Fetch nulls the Origin of a non-CORS non-`GET` request under that policy). A CSRF gate on the decision endpoint that compares `Origin` against an allow-list then 403s every approval — the flow dies at the last click, for every MCP client at once, and `fetch()`-based smoke tests never reproduce it. Accept `Sec-Fetch-Site: same-origin` at that gate, or serve the page `same-origin` policy instead (still no Referer cross-origin, Origin intact). See `skills/security-best-practice/rules/csrf-protection.md`.
...
- If the client is confidential (`tokenEndpointAuthMethod !== "none"` — that one field, not the presence of `clientSecretHash`), authenticate it **through the method it registered and no other**: `client_secret_basic` reads the `Authorization: Basic` header, and a secret arriving in the form body instead is `invalid_client`, not a second chance (if you also support `client_secret_post`, the same rule holds in reverse — each client is checked only through its own registered channel). Compare `SHA256(presented_secret)` to `clientSecretHash` with a constant-time comparison. A confidential client whose stored hash is missing is a broken row, not a public client: fail closed. Treat an empty credential value as omitted, and reject any request presenting more than one client-authentication method (a Basic header *and* a body `client_secret`, or a secret *and* a `client_assertion`) — RFC 6749 §2.3 forbids it, and accepting both lets the caller choose which credential you check. Status codes follow the method the client *used* (RFC 6749 §5.2): a failed Basic authentication is `401` with `WWW-Authenticate: Basic realm="oauth"` and the body `{"error":"invalid_client"}`; a failed body authentication is `400 invalid_client`. Basic credentials are form-urlencoded *before* base64 (RFC 6749 §2.3.1) — percent-decode both halves after decoding, and if a body `client_id` is also present it must equal the Basic username.
...
- **Optional reuse window for racing native clients.** Strict rotation is the default, and the right one. But desktop MCP clients are the population that hits the honest-retry case: two processes (or one client that lost the response to a network blip) present the same old refresh token seconds apart, and strict handling revokes the family and sends the user back through consent. If that is your traffic, implement the *replay-the-response* variant, not a relaxation: at rotation, store the token response you returned (encrypted at rest — it holds the new refresh token) on the consumed row together with a fingerprint of the request; a second presentation of the consumed token within a short window (better-auth's MCP plugin defaults to 30 s; keep it under a minute) **from the same client, for the same scopes and `resource`, and — if tokens are sender-constrained — the same DPoP key** gets that identical cached response back with `expires_in` recomputed. Never mint a second pair. A mismatched retry inside the window is `invalid_grant` *without* revoking the family; a presentation after the window is a replay and revokes the family as above. Replay only when the consumed row is not revoked (revocation outranks rotation state) and its replacement is itself still unconsumed and unrevoked; otherwise `invalid_grant`. Delete the cached response the moment the window closes (a sweep, or on next touch) — do not keep encrypted plaintext tokens for the 30-day life of the row; this is the single exception to the tokens-hashed-at-rest rule below and it is bounded to the window. Be honest about the cost: inside the window a thief holding the old token, presenting the same `client_id`/scopes/`resource` (trivial for a public client), receives the live pair and no family revocation fires — strict rotation would have burned both. Sender-constraining the fingerprint (DPoP, below) is what closes that blind spot. Whichever you pick, the bearer middleware and `/oauth/revoke` are unaffected — the window only decides how a repeated `refresh_token` grant answers.
...
- If you chose JWT access tokens (see **JWT vs opaque tokens** below), this spec assumes no `jti` denylist — revocation of a JWT is its short TTL plus the `sid` liveness check in the bearer middleware — so a JWT that still verifies against your own keys cannot be revoked server-side and there is no row to flag. Answer `400 unsupported_token_type` (RFC 7009 §2.2.1) rather than a `200` that pretends something happened, so the client learns nothing was revoked and revokes the refresh token instead; an expired or foreign JWT is just an unknown token and gets the usual 200 no-op. (If you do run a `jti` denylist, insert and return 200 — and accept that the middleware now hits the store on every request, which was the reason not to choose JWTs.) Refresh tokens and opaque access tokens revoke normally.
...
- **Tokens are hashed at rest.** Store `SHA256(token)` only. The plaintext goes back to the client once. Same rule as API keys. The one bounded exception is the optional refresh replay window: an *encrypted* copy of a token response held on the consumed refresh row for the life of the window and deleted when it closes.
- **Codes and tokens are single-use where applicable.** Authorization codes: one exchange and they're done. Refresh tokens: rotate on every use; replaying a consumed refresh token revokes the family (or, inside the optional replay window described under the refresh grant, returns the cached response — never a second pair).
...
- **Credential-bearing responses are `no-store`.** `/oauth/token` and `/oauth/register` MUST send `Cache-Control: no-store` and `Pragma: no-cache` — RFC 6749 §5.1 requires it for any response carrying tokens, credentials, or other sensitive information, which covers the freshly minted `client_secret` too (RFC 7591's examples show the same header) — on the `invalid_grant` / `invalid_client` error responses as well as on success, since those leave the same endpoint. Give `/oauth/revoke` and any introspection, userinfo, or device-authorization endpoint you add the same header; none of them returns anything a proxy should keep. Attach the headers where the route is declared, not at each `return`, and assert them in a test: a framework's dispatch or serialization layer will silently swallow a status code and headers set at the call site, which is how a freshly minted `client_secret` ends up in a corporate proxy's cache. The two `.well-known` documents are the only cacheable responses here (`max-age=3600`, above).
...
- **JWT vs opaque tokens.** Opaque tokens (this spec) trade slightly more DB load at the resource for instant revocation and no JWKS infrastructure. JWTs would let the MCP server validate without a DB hit but require JWKS rotation and give up instant revocation (a `jti` denylist buys it back at the price of the DB hit you were avoiding). For a self-hosted single-deployment MCP server, opaque is the right default. If you do choose JWT: use `typ: at+jwt` (RFC 9068 §2.1); embed `aud` matching the canonical resource URI and verify it on every request; keep the TTL short and put the session id in `sid` — with no `jti` denylist, that liveness check plus the TTL *is* revocation, which is why `/oauth/revoke` answers `unsupported_token_type` for a live JWT; emit `jti`; and treat the protocol claim names as **server-owned** — `iss`, `sub`, `aud`, `exp`, `nbf`, `iat`, `jti` (RFC 7519), `client_id`, `scope`, `auth_time`, `acr`, `amr` (RFC 9068 §2.2), `cnf` (RFC 7800 / RFC 9449), `sid` (OIDC), and for ID tokens also `nonce`, `at_hash`, `c_hash`, `azp`. Any hook that lets application code add claims strips those names (log it) instead of letting a "custom claim" override the audience or expiry the server just computed; namespaced custom claims (`https://example.com/org`) are the escape hatch.
- **Sender-constrained tokens (DPoP, RFC 9449) — optional hardening.** Everything above issues *bearer* tokens: whoever holds one can use it. DPoP binds the token to a key the client holds, so a token copied out of a desktop client's storage or a proxy log is useless without the private key. The MCP 2026-07-28 spec does not mention DPoP and still mandates that clients send `Authorization: Bearer` — a DPoP profile for MCP (SEP-1932) is proposed but unmerged — while better-auth 1.7's MCP package accepts DPoP-bound tokens; so treat it as a **per-client opt-in**, never a resource-wide requirement: bind a token only when the token request itself carries a `DPoP` proof header (RFC 9449 §5) or the client registered `dpop_bound_access_tokens: true`, and do not set `dpop_bound_access_tokens_required` on the PRM, which would lock out every compliant MCP client. If you implement it, the resource-server checks are all-or-nothing: (1) the request carries `Authorization: DPoP <token>` and exactly **one** `DPoP` header holding a JWT with `typ: dpop+jwt`, an asymmetric `alg` from the `dpop_signing_alg_values_supported` you advertise in **both** the AS metadata (RFC 9449 §5.1) and the PRM (RFC 9728 §2) — never `none`, never HMAC — and the public key in the `jwk` header, which must carry no private material; (2) its signature verifies with that key; (3) `htm` equals the request method and `htu` equals the request URL *without* query or fragment — your configured public origin plus the request path (RFC 3986-normalized before comparison), not the URL a proxy handed you (`references/pitfalls/oauth-redirect-request-url.md`); (4) `iat` is within a short window (~5 min) and `jti` has not been seen before — the replay store must be **shared** across instances (database-backed; an in-memory set is single-instance only), and if you cannot provide one, reject DPoP requests rather than skipping the check (a server-issued `nonce` is the RFC's optional alternative to a wide `iat` window; relying on `iat` + `jti` is fine); (5) `ath` equals the base64url SHA-256 of the presented access token; (6) the token's confirmation — `cnf.jkt` in a JWT, or the stored thumbprint on an opaque row — equals the RFC 7638 SHA-256 JWK thumbprint of the proof's `jwk` (not a hash of the serialized JSON). A bad proof is `401 invalid_dpop_proof`; a bound token presented as plain `Bearer`, or a thumbprint mismatch, is `401 invalid_token`; the `WWW-Authenticate` challenge on 401 names `DPoP algs="…"` alongside `Bearer`. On the AS side, the token endpoint validates the same proof shape (no `ath`), answers a bad proof with `400 invalid_dpop_proof`, stamps the thumbprint onto the access token, refresh token and family, and returns `token_type: DPoP` (not `Bearer`) — the client picks its scheme from that field. Refresh rotation then demands a proof from the same key: RFC 9449 makes that a MUST for public clients only, and this spec (like upstream) binds confidential clients' refresh tokens too, at the cost of a re-authorization on key rotation. Do not build DPoP halfway — an `htu` check that trusts `request.url` behind a TLS terminator, or a `jti` cache per instance, are exactly the two deployment caveats better-auth's own upgrade guide calls out.
