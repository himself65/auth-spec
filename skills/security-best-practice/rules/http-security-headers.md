...
| `Referrer-Policy` | `strict-origin-when-cross-origin` (general) or `no-referrer` (auth pages, reset links) — prevents leaking URLs with tokens. **`no-referrer` also nulls the `Origin` of any HTML form the page navigates with**: per Fetch, a non-CORS non-`GET` request's Origin serializes to `null` under this policy. So a CSRF gate on that form's target that compares `Origin` against an allow-list will 403 your own consent and reset submissions, and `new URL('null')` throws on top of it. Either pair this with the `Sec-Fetch-Site: same-origin` accept in `csrf-protection.md`, or use `same-origin` policy on those pages — it still sends **no** Referer cross-origin (including across the redirect that follows the POST), while leaving the same-origin `Origin` intact. `fetch()` callers never see this; only plain form navigations do. |
