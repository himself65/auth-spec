...
| Atomic check-and-increment | The limiter bumps the counter and reads the result in **one** operation (`INCR` + `EXPIRE` on creation via a Lua script, an upsert with `RETURNING` — `LAST_INSERT_ID(value + 1)` on MySQL — an in-process critical section) and decides on the returned value. A `get` → compare → `set(count + 1)` limiter lets a burst of concurrent requests all read the same stale count and all pass — it fails against exactly the flood it exists for (better-auth 1.7 dropped support for `get`/`set`-shaped rate-limit stores for this reason). Set the TTL only when the key is created; refreshing it on every hit means a window under sustained attack never closes. |
...
- The same atomic-increment rule applies to every counter that gates anything — OTP attempts, 2FA `failedVerificationCount`, lockout counters — whether it lives on a database row (`SET attempts = attempts + 1 … RETURNING attempts`) or in the KV store.
