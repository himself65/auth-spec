...
| Email validation | **Canonicalize first, validate second** (CWE-180): NFKC-normalize, trim, and lowercase, *then* check a **strict** shape — a lowercase RFC 5322 dot-atom local part, `[a-z0-9-]` domain labels, exactly one `@`, ≤ 254 chars, no `( ) < > [ ] , ; : \ "` or space anywhere, no control chars or CRLF — and store and mail exactly the string that passed. Validating the raw input lets a fullwidth `＠` (U+FF20) through a single-`@` check; a mail library that normalizes recipients then sees two separators and can deliver the sign-in link to a mailbox you never checked (Auth.js GHSA-7rqj-j65f-68wh, rated account takeover). A permissive "printable ASCII" check is not enough either: a mailer re-parses `a(b)@x` or `x<attacker@evil>` into a different recipient. Reject non-ASCII outright unless you deliberately support internationalized addresses — mandatory where the stdlib has no NFKC. Strip `+` aliases only if your policy says so. |
...
| Unicode normalization | Normalize usernames/emails (NFKC) **before validation**, storage, and comparison to prevent homoglyph account duplicates and validate-before-canonicalize bypasses. Any check that runs on the pre-normalized string proves nothing about the string a downstream library will see. |
...
```typescript
// GOOD: parameterized query + Zod schema; the email is canonicalized (NFKC, trim,
// lowercase) BEFORE it is validated — never `.email().toLowerCase()`, which validates the raw input
import { z } from 'zod';
const SignInBody = z.object({
  email: z.string()
    .transform((s) => s.normalize('NFKC').trim().toLowerCase())
