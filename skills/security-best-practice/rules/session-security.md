...
| Session freshness (sudo mode) | Record when the session last authenticated and require *recent* authentication (e.g. ≤ 15 min, else re-prompt for password/MFA) before sensitive operations: password/email change, MFA enroll/remove, revoking other sessions, account unlink/delete, API-key creation. Without this, any stolen long-lived cookie can silently rotate the account's credentials into the attacker's hands. Enforce the freshness check server-side on every sensitive endpoint — including the read/list endpoints that gate those flows — not just in the UI. Store `authenticatedAt` (and every session/token timestamp) in a real timestamp type, not text: a freshness comparison against a string column silently misparses (timezone, format drift) and either never re-prompts or re-prompts on every request — the "login loop" better-auth's 1.7 `max_age` enforcement documented for text-typed session dates. If you are an OIDC provider, this is also what honours a client's `max_age`: a login older than the request allows sends the user back through authentication instead of being ignored. |
| Refresh token rotation | If using refresh tokens, rotate on every use and detect **reuse** — if an old refresh token is replayed, revoke the entire family (indicates theft). The one sanctioned softening for native clients that genuinely race their own refreshes is a *replay-the-same-response* window of well under a minute: the consumed token, presented again by the same client for the same scopes/audience and the same sender-constraining key, gets the identical cached response back — never a second fresh pair; a mismatch inside the window is `invalid_grant` without family revocation, and a presentation after the window is still theft. |
| Sender-constrained tokens (DPoP) | Every token above is a *bearer*: whoever holds it can use it. Where public clients (desktop, CLI, mobile) hold long-lived tokens, consider DPoP (RFC 9449), which binds the token to a client-held key so a copied token is useless without it. Accepting DPoP is all-or-nothing on the resource server: exactly one `DPoP` header, `typ: dpop+jwt`, an advertised asymmetric `alg`, signature by the header `jwk` (public material only), `htm`/`htu` against the method and your **configured** public URL plus path, query and fragment ignored (not a proxied `request.url`), fresh `iat` and an unseen `jti` in a **shared** replay store (or a server-issued `DPoP-Nonce` you then require), `ath` equal to the hash of the presented token, the token's `cnf.jkt` equal to the RFC 7638 thumbprint of the proof key — and a DPoP-bound token offered under the plain `Bearer` scheme is rejected, never accepted as a downgrade. A half implementation is worse than none. |
| JWT-specific checks | `alg` must be validated on the server — never trust the header. Disable `alg: none`. Prefer EdDSA or RS256/ES256 over HS256 for asymmetric use cases. Validate `iss`, `aud`, `exp`, `nbf`, `iat`. Pin the algorithm in configuration and check at **startup** that the configured key material actually matches it (an EC key with `alg: RS256`, a key with no private material) — fail the boot, not the first token. When application code can add claims to a token you mint, the protocol claims (at least `iss`, `sub`, `aud`, `exp`, `nbf`, `iat`, `jti`, `nonce`, `sid`, `cnf`, `auth_time`, `acr`, `amr`, `azp`, `at_hash`, `c_hash`) are the server's: strip them from the hook's output with a warning rather than letting a "custom claim" rewrite the audience or lifetime you just computed. |
...
| Cached session snapshots | Caching a session to skip the store read — signed cookie payload, Redis/KV mirror, in-process map — buys a revocation lag equal to the cache TTL: a session signed out, a user banned, or a role demoted keeps passing every check that reads the snapshot until it expires. The trap is the *hybrid* deployment, where a durable store exists so the app believes revocation is instant. Make the store read the **default** and the cached read an explicit opt-in, so a newly added endpoint is safe by omission — only low-consequence reads may opt in. Anything that changes state, elevates privilege, touches credentials, or is an administrative action (admin/impersonation gates, permission checks, account deletion, API-key management) must re-read the durable store. Verify the snapshot's embedded expiry, not just its signature — a well-signed expired snapshot must not read as live. **Bind the snapshot to the session it caches**: put the session id (or a hash of the session token) inside the signed payload and require it to match the session cookie presented alongside — a snapshot presented with a different or missing session cookie is ignored, or a stale signed blob from a previous session on the same browser (or one copied from another) keeps reading as signed in after that session was rotated or revoked (better-auth 1.7 tied its cookie cache to the `session_token` cookie for this reason). Drop the cached entry in the same operation that revokes a session, changes a password, bans a user, or changes a role — and if that operation runs inside a database transaction, purge *after* it commits, and never **write** a snapshot from inside a transaction that may still roll back (a cache refreshed from an uncommitted update publishes a state the database never held). If any other service verifies these snapshots, sign them with an asymmetric key published at your JWKS so verifiers hold no minting capability — a shared HMAC secret makes every verifier a session forger. |
